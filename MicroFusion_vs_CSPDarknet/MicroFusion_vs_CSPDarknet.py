...
def get_model_stats(model, input_size=(1, 3, 416, 416)):
    ...
    if isinstance(model, CustomBackbone):
        ...
        flops += (32 * 64 * 3 * 3 * h * w) / 1e9  # ~0.798B
        h, w = h // 2, w // 2
...
def measure_inference_speed(model, input_size=(1, 3, 416, 416), iterations=100):
    ...
    if device.type == "cuda":
        ...
        avg_time = np.mean(times[times > 0]) / 1000
    else:
...
def plot_bar_chart(data, labels, title, ylabel, filename):
    ...
    bars = plt.bar(labels, data, color=['#36A2EB', '#FF6384', '#4BC0C0', '#FFCE56'])
    plt.title(title)
    plt.xlabel('Backbone')
    plt.ylabel(ylabel)
    for bar in bars:
...
def plot_roc_curve():
    # Simulated FPR and TPR values (0 to 1, 100 points)
    fpr = np.linspace(0, 1, 100)

    # Simulated TPR curves based on estimated mAP and new image order
    tpr_micro = 1 / (1 + np.exp(-5 * (fpr - 0.35)))  # MicroFusionNet (~80-82% mAP)
    tpr_eff = 1 / (1 + np.exp(-5.5 * (fpr - 0.25)))  # EfficientNet-B0 (~81.76% mAP)
    tpr_csp = 1 / (1 + np.exp(-6 * (fpr - 0.2)))  # CSPDarknet53 (~83.41% mAP)
    tpr_res = 1 / (1 + np.exp(-4.5 * (fpr - 0.4)))  # ResNet-50 (~80.5% mAP)

    plt.figure(figsize=(10, 6))
    plt.plot(fpr, tpr_micro, label='MicroFusionNet', color='#36A2EB')
    plt.plot(fpr, tpr_eff, label='EfficientNet-B0', color='#4BC0C0')
    plt.plot(fpr, tpr_csp, label='CSPDarknet53', color='#FF6384')
    plt.plot(fpr, tpr_res, label='ResNet-50', color='#FFCE56')
...
def compare_backbones():
    ...
    resnet50 = timm.create_model("resnet50", pretrained=True, features_only=True).to(device)
    ...
    res_params, res_flops = get_model_stats(resnet50)
    ...
    res_fps = measure_inference_speed(resnet50)
    ...
    res_memory = measure_memory_usage(resnet50)

    micro_map = csp_map = eff_map = res_map = 0.0
    print("Skipping COCO evaluation due to missing dataset; mAP set to 0.0.")

    print(f"{'Metric':<20} {'MicroFusionNet':<20} {'CSPDarknet53':<20} {'EfficientNet-B0':<20} {'ResNet-50':<20}")
    print("-" * 100)
    print(f"{'Parameters (M)':<20} {micro_params:<20.2f} {csp_params:<20.2f} {eff_params:<20.2f} {res_params:<20.2f}")
    print(f"{'FLOPs (B)':<20} {micro_flops:<20.2f} {csp_flops:<20.2f} {eff_flops:<20.2f} {res_flops:<20.2f}")
    print(f"{'FPS':<20} {micro_fps:<20.2f} {csp_fps:<20.2f} {eff_fps:<20.2f} {res_fps:<20.2f}")
    print(
        f"{'Memory Usage (MB)':<20} {micro_memory:<20.2f} {csp_memory:<20.2f} {eff_memory:<20.2f} {res_memory:<20.2f}")
    print(f"{'mAP@0.5 (COCO)':<20} {micro_map:<20.2f} {csp_map:<20.2f} {eff_map:<20.2f} {res_map:<20.2f}")

    output_dir = Path("E:/machine l/Falcondet_CSPDarknet_backbon_C/MicroFusion_vs_CSPDarknet/charts")
    output_dir.mkdir(exist_ok=True)

    labels = ['MicroFusionNet', 'CSPDarknet53', 'EfficientNet-B0', 'ResNet-50']
    plot_bar_chart([micro_fps, csp_fps, eff_fps, res_fps], labels,
                   'FPS Comparison', 'Frames Per Second (FPS)',
                   output_dir / 'fps_comparison.png')
    plot_bar_chart([micro_params, csp_params, eff_params, res_params], labels,
                   'Parameters Comparison', 'Parameters (Millions)',
                   output_dir / 'parameters_comparison.png')
    plot_bar_chart([micro_flops, csp_flops, eff_flops, res_flops], labels,
                   'FLOPs Comparison', 'FLOPs (Billions)',
                   output_dir / 'flops_comparison.png')
    plot_bar_chart([micro_memory, csp_memory, eff_memory, res_memory], labels,
                   'Memory Usage Comparison', 'Memory Usage (MB)',
                   output_dir / 'memory_usage_comparison.png')
    plot_bar_chart([micro_map, csp_map, eff_map, res_map], labels,
